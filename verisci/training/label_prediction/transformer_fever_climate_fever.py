...
class FeverLabelPredictionDataset(Dataset):
    def __init__(self, file):
        ...
        for data in jsonlines.open(file): 
            # if data['label'] == 'NOT ENOUGH INFO': #TODO 这种情况，大多数也是有evidence_set的。没有evidence_set的例子跳过（这种本来就会被直接标注为not)
            #     if data['sentences']:
            ...
            #         })
            #         #    'rationale': ' '.join([data['sentences'][i[0]] for i in data['evidence_sets']]),
